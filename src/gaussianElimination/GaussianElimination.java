...
public class GaussianElimination {
	...
	public void userInput() {
		...
		for (int counter = 0; counter < sizeX * sizeY; counter++) {
			...
			if (j == sizeX && i < sizeY - 1) // If j is the same value as the
												// furthest right column, and i
												// (row placekeeper) is not in
	...
	public void printProcessed() {
		...
		for (int i = 0; i < sizeY; i++) {
			for (int j = 0; j < sizeX; j++) {
				...
				System.out.print((Math.ceil(anArray[i][j] * 100) / 100) + " ");
			}
			System.out.println("");
		}
		System.out.println("");
...

	{
		...
		if (anArray[(sizeY - 1)][(sizeX - 1)] == 0 && anArray[(sizeY - 1)][(sizeX - 2)] == 0) // If
																								// 0s
																								// in
																								// bottom
																								// row
		{
			...
			for (int x = 0; x <= (sizeY - 1); x++) {
				System.out.println("x" + (x + 1) + " = " + (Math.ceil(resultsArray[x] * 100)) / 100);
			}
		}
	}
}